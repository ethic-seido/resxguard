...
fn repair_updates_values_and_keys() {
    ...
    Command::cargo_bin("resxguard")
        ...
        .arg(&resx)
        .args(["--mode", "safe", "--backup"])
        .assert()
        .success();

    let updated = fs::read_to_string(&resx).unwrap();
    assert!(updated.contains("Créez un visuel avec l’éditeur"));
    assert!(updated.contains("ImageStudio_Éditeur"));
    assert!(
...
fn import_can_continue_on_error() {
    ...
    Command::cargo_bin("resxguard")
        ...
        .arg(&resx)
        .arg(fixture("translations-mixed.fr.csv"))
